...
export function ConditionEditor({project,value,onChange,actor=false}:{project:GameProject;value:Condition[];onChange:(v:Condition[])=>void;actor?:boolean}){
 ...
 {['target','ready'].includes(c.kind)&&<Select label={'Capacité de condition '+(i+1)} value={c.ref} change={ref=>update(i,{...c,ref})} options={[{id:'',name:'Une capacité active'},...project.combat!.abilities]}/>}
...
export function ActionEditor({project,value,onChange,self=false}:{project:GameProject;value:LogicAction[];onChange:(v:LogicAction[])=>void;self?:boolean}){
 ...
 {a.abilityId!==undefined&&<Select label={'Capacité de l’action '+(i+1)} value={a.abilityId} change={abilityId=>patch(i,{abilityId})} options={project.combat!.abilities}/>}
 {a.type==='use_ability'&&<p className="gd-note">La capacité doit être attribuée, prête et disposer d’une cible. Aucun délai de recharge n’est contourné.</p>}
 {a.type==='set_ability'&&<label className="logic-check"><input type="checkbox" checked={a.enabled} onChange={e=>patch(i,{enabled:e.target.checked})}/>Capacité activée</label>}
 {a.type==='apply_effect'&&<Select label={'Effet à appliquer '+(i+1)} value={a.effectId!} change={effectId=>patch(i,{effectId})} options={project.combat!.effects}/>}
