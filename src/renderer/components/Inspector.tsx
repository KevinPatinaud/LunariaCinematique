  ...
  return <aside className="inspector panel">
    ...
    <fieldset disabled={p.disabled || p.objectLocked} className="inspector-content">
      ...
      {!actor && !bubble && (p.multiCount??0)<=1 && <>
        ...
        <div className="section-label"><Icon name="play" size={15}/> PASSAGE AU PLAN SUIVANT</div>
        <div className="shot-end-choice" role="group" aria-label="Passage au plan suivant">
          <button type="button" className={p.shot.endAdvance !== 'click' ? 'active' : ''} aria-pressed={p.shot.endAdvance !== 'click'} onClick={() => p.updateShot({ endAdvance: 'auto' }, 'end-advance')}><strong>Enchaîner automatiquement</strong><small>Après la durée minimale et toutes les répliques.</small></button>
          <button type="button" className={p.shot.endAdvance === 'click' ? 'active' : ''} aria-pressed={p.shot.endAdvance === 'click'} onClick={() => p.updateShot({ endAdvance: 'click' }, 'end-advance')}><strong>Attendre un clic</strong><small>Le joueur confirme la fin de ce plan.</small></button>
        </div>
        <p className="field-hint">Chaque réplique possède aussi son propre choix de lecture automatique ou par clic.</p><Field label="Début des dialogues (s)"><NumberInput label="Début des dialogues" min={0} max={p.shot.duration} value={p.shot.dialogueStart} onCommit={dialogueStart => p.updateShot({ dialogueStart }, 'dialogue-start')}/></Field>
