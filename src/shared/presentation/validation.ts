...
export function presentationIssues(p:GameProject):GameIssue[]{
 ...
 for(const a of c.animations){const path='presentation.animations.'+a.id,d=duration(a);if(a.ownerSpeciesId){if(!species(p,a.ownerSpeciesId))add(path,'Personnage propriétaire introuvable : '+a.ownerSpeciesId);for(const pr of c.profiles){if(!pr.slots.some(s=>s.animationId===a.id))continue;if(pr.id===c.defaultProfileId||[...p.balance.plants,...p.balance.enemies].some(s=>s.animationProfileId===pr.id&&s.id!==a.ownerSpeciesId))add(path,'Animation propre à un personnage utilisée par un autre personnage ou le profil commun.');}}if(a.kind!=='procedural'&&!a.frames.length)add(path,'Une séquence doit contenir au moins une frame.');if(a.kind==='procedural'&&a.frames.length)add(path,'Utilise le mode Combinée pour associer images et transformation.');if(!(d>0&&d<=120))add(path,'Durée totale attendue entre 0 et 120 secondes.');
