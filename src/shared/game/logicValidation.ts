...
export function logicIssues(p:GameProject):GameIssue[]{
 ...
 if(!p.combat){add('combat','Catalogue de capacités absent.');return issues;}
 ...
 function conditions(list:Condition[],at:string,actor=false){
  for(const c of list){if(['health','ready','target'].includes(c.kind)&&!actor)add(at,'Cette condition nécessite un porteur : utilise un déclencheur de PV ou une règle de comportement.');
   if(['ready','target'].includes(c.kind)&&c.ref&&!abilities.has(c.ref))add(at,'Capacité de condition introuvable.');
 ...
 function actions(list:LogicAction[],at:string,actor=false,team=''){
  ...
  for(const a of list){elapsed+=a.delay;const fields=ACTION_FIELDS[a.type]??[];
   ...
   if(['use_ability','set_ability'].includes(a.type)&&!abilities.has(a.abilityId!))add(at,'Capacité de l’action introuvable.');
   if(a.type==='apply_effect'&&!effects.has(a.effectId!))add(at,'Effet de l’action introuvable.');
   ...
   if(['use_ability','set_ability'].includes(a.type)&&a.target!=='self'){
    ...
    if(!matches.some(s=>possibleAbilities(p,s).includes(a.abilityId!)))add(at,'Aucune espèce ciblée ne possède cette capacité.');
   }
 ...
 for(const b of p.logic.behaviors){const at='logic.behaviors.'+b.id;unique(b.id,at);
  ...
  for(const [i,f] of b.phases.entries()){
   ...
   if(f.abilityIds.some(id=>!abilities.has(id)))add(at,'Capacité de phase introuvable.');if(new Set(f.abilityIds).size!==f.abilityIds.length)add(at,'Capacité répétée dans une phase.');actions(f.onEnter,at,true,b.team);
  }
  const owners=species.filter(s=>s.behaviorId===b.id);
  for(const r of b.rules){unique(r.id,at);conditions(r.conditions,at,true);if(r.action==='ability'&&r.abilityId&&!abilities.has(r.abilityId))add(at,'Capacité de règle inconnue.');if(r.action!=='ability'&&r.abilityId)add(at,'Une règle de déplacement ne lance pas une capacité.');
   if(r.abilityId&&owners.some(s=>!possibleAbilities(p,s).includes(r.abilityId)))add(at,'La capacité demandée n’est pas attribuée à tous les porteurs du comportement (ni dans ses phases).');
  }
  for(const f of b.phases)for(const a of f.onEnter)if(a.target==='self'&&a.abilityId&&owners.some(s=>!possibleAbilities(p,s).includes(a.abilityId!)))add(at,'Une action de phase demande une capacité non attribuée au porteur.');
 }
 for(const [team,list] of [['plants',p.balance.plants],['enemies',p.balance.enemies]] as const)for(const s of list){const at='balance.'+team+'.'+s.id,b=p.logic.behaviors.find(b=>b.id===s.behaviorId);if(!b||b.team!==team)add(at,'Comportement absent ou prévu pour un autre camp.');if(possibleAbilities(p,s).length>24)add(at,'Maximum 24 capacités, phases comprises.');}
 for(const l of p.levels){const at='levels.'+l.id;if(!Array.isArray(l.events)){add(at,'Liste des événements absente.');continue;}
  for(const ev of l.events){const ep=at+'.events.'+ev.id;unique(ev.id,ep);conditions(ev.conditions,ep);actions(ev.actions,ep);
   ...
   if(t.kind==='ability_used'&&!abilities.has(t.ref))add(ep,'Capacité du déclencheur introuvable.');
