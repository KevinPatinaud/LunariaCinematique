...
test('new level contains references, not embedded stats',()=>{const l=newLevel(['radish','bramble'],'litterer');assert.deepEqual(l.allowedPlants,['radish','bramble']);assert.equal(totalEnemies(l),5);assert.equal('damage' in l,false);good(p=>p.levels=[l]);});
