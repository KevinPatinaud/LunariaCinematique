...
test('click at the end of a plan is separate from automatic dialogue', () => { const doc=newCinematic(); doc.shots[0].duration=1; doc.shots[0].dialogueStart=0; doc.shots[0].endAdvance='click'; const bubble=newBubble(); bubble.advance={mode:'auto',seconds:.5}; doc.shots[0].bubbles=[bubble]; doc.shots.push({...duplicateShot(doc.shots[0]),endAdvance:'auto',bubbles:[]}); let state=tickPlayback(doc,beginPlayback(),2); assert.equal(state.shotIndex,0); assert.equal(state.dialogueIndex,1); assert.equal(state.finished,false); state=advanceDialogue(doc,state); assert.equal(state.shotIndex,1); assert.equal(state.elapsed,0); state=tickPlayback(doc,state,1); assert.equal(state.finished,true); });
test('empty plan can wait for a click and ignores early clicks', () => { const doc=newCinematic(); doc.shots[0].duration=1; doc.shots[0].endAdvance='click'; let state=tickPlayback(doc,beginPlayback(),.5); assert.deepEqual(advanceDialogue(doc,state),state); state=tickPlayback(doc,state,2); assert.equal(state.finished,false); assert.equal(advanceDialogue(doc,state).finished,true); });
